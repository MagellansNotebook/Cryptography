...
class Encryption():
	...
	def xor_func(self,value,variables):
		...
		h2 = int(variables[1],2)^h1
		h3 = int(variables[2],2)^h2
		h4 = int(variables[3],2)^h3
		h5 = int(variables[4],2)^h4

		# converts the integer to a binary digit in a 32 bit length
		# bin() - converts to binary string
		# .zfill () - adds zero to the left until the it reaches its max length
		xor_result = bin(h5)[2:].zfill(32)

		# converts the binary to a hexadecimal value
		hex_result = hex(int(xor_result))[2:]

		print('\nThe Encrypted data is:',hex_result)
